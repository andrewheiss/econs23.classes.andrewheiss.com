...
The font used throughout the site is [IBM Plex Sans Condensted](https://fonts.google.com/specimen/IBM+Plex+Sans+Condensed).

The colors for the site and hex logo come from the [Aurora](https://www.nordtheme.com/docs/colors-and-palettes#aurora) and [Frost](https://www.nordtheme.com/docs/colors-and-palettes#frost) palettes from [Nord](https://www.nordtheme.com/):

```{r show-nord, fig.width=8, fig.height=1.5, echo=FALSE, warning=FALSE, message=FALSE}
library(tidyverse)
library(patchwork)

pal_frost <- c("#88c0d0", "#81a1c1", "#5e81ac")
pal_aurora <- c("#bf616a", "#d08770", "#ebcb8b", "#a3be8c", "#b48ead")

p1 <- tibble(
  x = 1:length(pal_frost),
  color = pal_frost
) %>%
...
(p1 | plot_spacer() | p2) +
  plot_layout(widths = c((0.12 * 3), 0.04, (0.12 * 5)))
```
