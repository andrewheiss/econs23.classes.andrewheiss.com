...
<iframe src="https://www.youtube.com/embed/JV5lueZynos" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen="" frameborder="0"></iframe>
</div>
...
```{r gains-from-trade, warning=FALSE, message=FALSE, echo=FALSE, fig.width=5.5, fig.height=5.5}
library(tidyverse)

source(here::here("R", "graphics.R"))

df_trade <- tribble(
  ~country, ~avocados_max, ~olive_max, 
  "Mexico", 5,             2,
  "Italy",  1,             5
) %>%
  mutate(avocados_min = 0, olive_min = 0,
         country = fct_inorder(country, ordered = TRUE))

ggplot(df_trade) +
  geom_segment(aes(x = olive_max, y = avocados_min,
                   xend = olive_min, yend = avocados_max, color = country),
               size = 2) +
  labs(x = "Tons of olive oil", y = "Tons of avocados") +
  scale_color_manual(values = c(nord_lt_blue, nord_red), name = NULL) +
  scale_x_continuous(expand = c(0, 0), limits = c(0, 6), breaks = seq(0:6)) + 
  scale_y_continuous(expand = c(0, 0), limits = c(0, 6), breaks = seq(0:6)) +
  theme_econ(13) +
...
```{r expanded-gains, warning=FALSE, message=FALSE, echo=FALSE, fig.width=5.5, fig.height=5.5}
df_trade_expanded <- tribble(
  ~country, ~avocados_max, ~olive_max, 
  "Mexico", 5,             2,
  "Italy",  1,             5,
  "1:1 trading", 5,        5
...
ggplot(df_trade_expanded) +
  geom_segment(aes(x = olive_max, y = avocados_min,
                   xend = olive_min, yend = avocados_max, color = country),
               size = 2) +
  labs(x = "Tons of olive oil", y = "Tons of avocados") +
  scale_color_manual(values = c(nord_lt_blue, nord_red, nord_orange), name = NULL) +
  scale_x_continuous(expand = c(0, 0), limits = c(0, 6), breaks = seq(0:6)) + 
  scale_y_continuous(expand = c(0, 0), limits = c(0, 6), breaks = seq(0:6)) +
  theme_econ(13) +
  theme(panel.grid = element_blank(),
...
```{r cr-mexico-gains, warning=FALSE, message=FALSE, echo=FALSE, fig.width=5.5, fig.height=5.5}
df_trade_cr <- tribble(
  ~country,     ~avocados_max, ~olive_max, 
  "Mexico",     5,             2,
  "Costa Rica", 3,             1
...
ggplot(df_trade_cr) +
  geom_segment(aes(x = olive_max, y = avocados_min,
               ...
               size = 2) +
  labs(x = "Tons of olive oil", y = "Tons of avocados") +
  scale_color_manual(values = c(nord_lt_blue, nord_purple), name = NULL) +
  scale_x_continuous(expand = c(0, 0), limits = c(0, 6), breaks = seq(0:6)) + 
  scale_y_continuous(expand = c(0, 0), limits = c(0, 6), breaks = seq(0:6)) +
  theme_econ(13) +
  theme(panel.grid = element_blank(),
